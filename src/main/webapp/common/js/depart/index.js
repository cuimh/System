...
function getUserInfo(departId){
	comn.ajax({
		url: "wyNLSc//info/depart/Depart.do",
		data:{
			departId:departId
		},
...
$("#saveFee1").click(function(){
	...
	if($("#addFeeForm").valid() == true){
		var departId = $("#departId").val() || departId;
		var data = $("#addFeeForm").values();
		var _url,_message;
		 if (!departId) {
	            _url = "wyNLSc//info/depart/add.do"; //新增
	            _message = "添加成功!";
	        } else {
	            _url = "wyNLSc//info/depart/Update.do";  //修改
	            _message = "修改成功!";
	        }
		comn.ajax({
			url:_url,
			data: $.extend(data,{departId: departId || $("#departId").val()
		}),
		success: function (res){
...
dataLoad_1 = function(params){
	...
	tableData(params,$.extend($("#fee-form").values(),p),"wyNLSc//info/depart/list.do");
};
...
"click .modify":function(e,a,item,index){
	$("#addFeeModal").modal("show");
	$("#feeBoxPanel").html("修改院系信息");
	 window.location.href = "#addFeeModal";
	 console.log(item);
	 getUserInfo(item['departid']);
	 $("#feeFieldset").removeAttr("disabled");
},
"click .del": function (e, a, item, index) {
    ...
    $("#sureBtn").unbind("click").click(function () {
        return comn.ajax({
            url: "wyNLSc//info/depart/deletedepart.do",
            data: {
