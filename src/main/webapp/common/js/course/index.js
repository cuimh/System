...
function getUserInfo(courseId){
	comn.ajax({
		url: "wyNLSc//info/course/Course.do",
		data:{
			courseId:courseId
		},
...
$("#saveFee1").click(function(){
	...
	if($("#addFeeForm").valid() == true){
		var courseId = $("#courseId").val() || courseId;
		var data = $("#addFeeForm").values();
		var _url,_message;
		 if (!courseId) {
	            _url = "wyNLSc//info/course/add.do"; //新增
	            _message = "添加成功!";
	        } else {
	            _url = "wyNLSc//info/course/Update.do";  //修改
	            _message = "修改成功!";
	        }
		comn.ajax({
			url:_url,
			data: $.extend(data,{courseId: courseId || $("#courseId").val()
		}),
		success: function (res){
...
"click .modify":function(e,a,item,index){
	$("#addFeeModal").modal("show");
	$("#feeBoxPanel").html("修改课程信息");
	 window.location.href = "#addFeeModal";
	 console.log(item);
	 getUserInfo(item['courseid']);
	 $("#feeFieldset").removeAttr("disabled");
},
"click .del": function (e, a, item, index) {
    ...
    $("#sureBtn").unbind("click").click(function () {
        return comn.ajax({
            url: "wyNLSc//info/course/deletecourse.do",
            data: {
